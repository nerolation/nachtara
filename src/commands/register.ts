...
import { deriveMainAddress } from '../lib/crypto.js';
import {
  printHeader,
  printSubheader,
  printSuccess,
...
export async function registerCommand(options: { force?: boolean }): Promise<void> {
...
export async function lookupCommand(address: string): Promise<void> {
  ...
  try {
    const metaAddress = await getRegisteredMetaAddress(address as `0x${string}`);
    stopSpinner(true);
