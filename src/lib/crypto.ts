 ...
 */
export function generateRandomKeys(): StealthKeys {
 ...
 */
export function createStealthMetaAddress(keys: StealthKeys): Hex {
 ...
 */
export function parseStealthMetaAddress(metaAddress: Hex): {
 ...
 */
export function isValidPublicKey(pubKey: Hex): boolean {
  const cleaned = pubKey.slice(2);
  if (cleaned.length !== 66) return false;
  if (!cleaned.startsWith('02') && !cleaned.startsWith('03')) return false;
 ...
 */
export function deriveMainAddress(spendingPublicKey: Hex): Address {
 ...
 */
export function createAnnouncementMetadata(viewTag: Hex, extraData?: Hex): Hex {
 ...
 */
export function deriveEncryptionKey(password: string, salt: Uint8Array): Uint8Array {
...
export async function decryptData(
  ...
  const salt = hexToBytes(saltHex as Hex);
  const iv = hexToBytes(ivHex as Hex);
  const encrypted = hexToBytes(encryptedHex as Hex);
  const key = deriveEncryptionKey(password, salt);
