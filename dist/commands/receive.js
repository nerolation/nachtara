...
import { getCurrentNetwork, fetchAnnouncements, getBalance, getBlockNumber } from '../lib/network.js';
import { checkStealthAddress, computeStealthPrivateKey, extractViewTag } from '../lib/crypto.js';
import { printHeader, printSubheader, printSuccess, printError, printWarning, printInfo, printStealthAddresses, printNetworkInfo, startSpinner, stopSpinner, updateSpinner } from '../ui/display.js';
...
export async function receiveCommand(options) {
    ...
    for (const announcement of announcements) {
        ...
        const isOurs = checkStealthAddress(announcement.ephemeralPubKey, wallet.keys.spendingPublicKey, wallet.keys.viewingPrivateKey, announcement.stealthAddress, viewTag);
        if (isOurs) {
            const privateKey = computeStealthPrivateKey(announcement.ephemeralPubKey, wallet.keys.spendingPrivateKey, wallet.keys.viewingPrivateKey);
            ownedAddresses.push({
...
export function getDiscoveredAddresses() {
