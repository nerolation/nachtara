...
import { getCurrentNetwork, getRegisteredMetaAddress, sendToStealthAddress, getBalance, getGasPrice } from '../lib/network.js';
import { generateStealthAddress, isValidPublicKey, parseStealthMetaAddress } from '../lib/crypto.js';
import { printHeader, printSubheader, printSuccess, printError, printInfo, printSendSummary, printTxHash, printKeyValue, printNetworkInfo, startSpinner, stopSpinner } from '../ui/display.js';
export async function sendCommand(options) {
    ...
    let recipientMetaAddress;
    if (options.meta) {
        // Direct meta-address provided
        recipientMetaAddress = options.meta;
    }
    else if (options.to) {
        // Lookup from registry
        const spinner = startSpinner('Looking up recipient...');
        try {
            const meta = await getRegisteredMetaAddress(options.to);
            stopSpinner(true);
            if (!meta) {
        ...
        if (inputType === 'address') {
            ...
            const spinner = startSpinner('Looking up recipient...');
            try {
                const meta = await getRegisteredMetaAddress(address);
                stopSpinner(true);
                if (!meta) {
                    printError('Recipient has no registered stealth meta-address.');
                    return;
