...
export function printWalletInfo(params) {
    ...
    table.push([chalk.gray('Address'), chalk.cyan(address)], [chalk.gray('Stealth Meta-Address'), chalk.white(truncateHex(stealthMetaAddress, 20))]);
...
export function printNetworkInfo(network) {
    const table = new Table({
        chars: {
            'top': '─', 'top-mid': '┬', 'top-left': '┌', 'top-right': '┐',
            'bottom': '─', 'bottom-mid': '┴', 'bottom-left': '└', 'bottom-right': '┘',
    ...
    table.push([chalk.gray('Network'), chalk.yellow(network.name)], [chalk.gray('Chain ID'), chalk.white(network.chainId.toString())], [chalk.gray('RPC URL'), chalk.cyan(truncateUrl(network.rpcUrl))]);
    if (network.blockExplorer) {
        table.push([chalk.gray('Explorer'), chalk.underline.blue(network.blockExplorer)]);
    }
    console.log(table.toString());
}
export function printStealthAddresses(addresses) {
...
export function printSendSummary(params) {
    ...
    table.push([chalk.gray('Recipient Meta-Address'), chalk.cyan(truncateHex(recipient, 15))], [chalk.gray('Generated Stealth Address'), chalk.cyan(stealthAddress)], [chalk.gray('Amount'), chalk.green(`${amount} ETH`)], [chalk.gray('Network'), chalk.yellow(network.name)]);
    console.log(table.toString());
}
export function printSecurityNotice() {
    console.log();
