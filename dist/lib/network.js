...
import { mainnet, sepolia, holesky, arbitrum, optimism, base, polygon } from 'viem/chains';
import { SUPPORTED_NETWORKS, ERC5564_ANNOUNCER, ERC6538_REGISTRY, STEALTH_FORWARDER, START_BLOCKS, SCHEME_ID } from './types.js';
 ...
 * Set active network.
 */
export async function setNetwork(networkName) {
 ...
 * Set custom RPC URL.
 */
export async function setCustomRpc(rpcUrl) {
    const config = await loadConfig();
 ...
 */
export async function clearCustomRpc() {
    const config = await loadConfig();
 ...
 */
export async function createPublicClientForNetwork() {
 ...
 */
export async function createWalletClientForNetwork(privateKey) {
    const network = await getCurrentNetwork();
    const chain = CHAINS[network.chainId] || {
        id: network.chainId,
        name: network.name,
 ...
 */
export async function getRegisteredMetaAddress(registrant) {
 ...
 */
export async function registerMetaAddress(stealthMetaAddress, privateKey) {
...
export async function sendToStealthAddress(params) {
    ...
    if (forwarderAddress) {
        const { request } = await publicClient.simulateContract({
            ...
            args: [stealthAddress, ephemeralPublicKey, viewTag],
            value: amountWei,
            account: walletClient.account
        });
